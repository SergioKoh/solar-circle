...
def draw_umbrella(x, y, width, height, width_umbrella, height_umbrella,color=(227, 130, 25),
    ...
    for xt, xb in xy_top:
        for i in range(3):
            line(screen, color_outline, (xt, y),
                 (xb + i * step * width_umbrella, y + height_umbrella), 3)
...
def draw_background(x, y, width, height, color=(161, 245, 255)):
    ...
    draw_cloud(0.22*width, 0.24*height, 0.09*min(height, width))
    draw_cloud(0.5*width, 0.33*height, 0.09*min(height, width))
...
def draw_middleground(x, y, width, height, color=(68, 35, 223)):
    ...
    draw_boat(0.59*width, y + 0.2*height, 0.24*width, 0.31*height, 0.1*width)
...
def draw_foreground(x, y, width, height, color=(224, 238, 12)):
    ...
    """
    rect(screen, color, (x, y, width, height))
    draw_umbrella(0.15 * width, y - 0.4*height, 0.011 * width,
