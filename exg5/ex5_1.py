...
class Window(tk.Tk):
    ...
    def drawing_targets(self, targets):
        """Drawing goals."""
        self.canvas.delete("text")
        self.canvas.delete("bum")
        for target in targets:
            target.id = self.canvas.create_oval(target.x - target.r, target.y - target.r,
                                                target.x + target.r, target.y + target.r, fill=target.color)
        ...
        for target in targets:
            if (target.r + self.r) ** 2 >= ((target.x + target.r - (self.x + self.r)) ** 2 +
                ...
                if not self.flag_hit:
                    ...
                    window.after(3000, target.new_targets)
                window.after_cancel(self.id_after)
...
class Target:
    ...
    def new_targets(self):
        ...
        window.drawing_targets(targets)
